...
export default function SummaryClient({ courseId }: { courseId: string }) {
  ...
  const exportCsv = () => {
    ...
    const header = [
      "#", "รหัส", "ชื่อ", "นามสกุล",
      ...sessions.map((s) => `${s.date} ค${s.period}`),
      "มาทั้งหมด", "ขาด", "สาย", "%",
    ];
    const rows = students.map((s) => {
      ...
      const cells = sessions.map((sess) => {
        ...
        if (!st) return "-";
        return st === "present" ? "✓" : st === "late" ? "S" : st === "absent" ? "—" : "⚠";
      });
      return [
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-gray-400 mt-0.5">
            {sessions.length} คาบ · {students.length} คน
          </p>
        </div>
      ...
      {/* Grid */}
      <div className="overflow-x-auto rounded-xl" style={{ border: "0.5px solid rgba(0,0,0,0.12)" }}>
        <table className="min-w-full text-sm border-collapse bg-white">
          <thead>
            <tr style={{ borderBottom: "0.5px solid rgba(0,0,0,0.10)" }}>
              <th className="sticky left-0 bg-gray-50 text-left px-2 py-2.5 text-xs font-medium text-gray-500 w-8">#</th>
              <th className="sticky left-8 bg-gray-50 text-left px-3 py-2.5 text-xs font-medium text-gray-500 min-w-[7rem]">รหัส</th>
              <th className="sticky left-[8.5rem] bg-gray-50 text-left px-3 py-2.5 text-xs font-medium text-gray-500 min-w-[9rem]">ชื่อ-นามสกุล</th>
              {sessions.map((ss) => (
                <th key={ss.session_id} className="px-2 py-2.5 text-xs font-medium text-gray-500 text-center min-w-[3.5rem]">
                  <div>{ss.date.slice(5)}</div>
                  <div className="text-gray-400 font-normal">ค{ss.period}</div>
                </th>
              ))}
              <th className="px-2 py-2.5 text-xs font-medium text-center min-w-[2.5rem]" style={{ color: "#3B6D11" }}>มา</th>
              <th className="px-2 py-2.5 text-xs font-medium text-center min-w-[2.5rem]" style={{ color: "#A32D2D" }}>ขาด</th>
              <th className="px-2 py-2.5 text-xs font-medium text-center min-w-[2.5rem]" style={{ color: "#185FA5" }}>สาย</th>
              <th className="px-2 py-2.5 text-xs font-medium text-gray-700 text-center min-w-[3rem]">%</th>
            </tr>
          </thead>
          <tbody>
            {students.map((stu) => {
              ...
              return (
                <tr key={stu.student_id} style={{ borderBottom: "0.5px solid rgba(0,0,0,0.06)", backgroundColor: rowBg }}>
                  <td className="sticky left-0 px-2 py-2 text-xs text-gray-400" style={{ backgroundColor: rowBg }}>{stu.order_num}</td>
                  <td className="sticky left-8 px-3 py-2 font-mono text-xs text-gray-600" style={{ backgroundColor: rowBg }}>{stu.student_id}</td>
                  ...
                  {sessions.map((ss) => {
                    ...
                    return (
                      <td key={ss.session_id} className="px-2 py-2 text-center">
                        ...
                        {st === "late"    && <span style={{ color: "#185FA5" }} className="text-xs font-bold">S</span>}
                        {st === "absent"  && <span style={{ color: "#A32D2D" }}>—</span>}
                        {st === "gps_fail"&& <span className="text-xs" style={{ color: "#854F0B" }}>⚠</span>}
                        {!st              && <span className="text-gray-200 text-xs">·</span>}
                      </td>
                    );
                  })}
